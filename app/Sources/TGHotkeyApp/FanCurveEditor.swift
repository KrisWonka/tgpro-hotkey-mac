...
/// 可拖拽折线图：横轴 20-90°C（5°C 一格），纵轴 0-100%（5% 一格）。
...
struct FanCurveEditor: View {
    ...
    private func pointHandles(in plot: CGRect) -> some View {
        ForEach(curve.indices, id: \.self) { i in
            ...
            Circle()
                ...
                // 旁边显示当前值
                .overlay(
                    Text("\(Int(cp.temperatureLimit))° \(cp.percent)%")
    ...
    private func yPos(_ p: Double, in plot: CGRect) -> CGFloat {
        let f = (p - percentLow) / (percentHigh - percentLow)
        return plot.maxY - plot.height * CGFloat(min(max(f, 0), 1))
    }
    ...
    private func percent(at y: CGFloat, in plot: CGRect) -> Double {
        ...
        return percentLow + f * (percentHigh - percentLow)
    }
